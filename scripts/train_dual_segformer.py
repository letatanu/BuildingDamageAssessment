'''
CUDA_VISIBLE_DEVICES=6,7 torchrun --nproc_per_node=2 -m scripts.train_dual_segformer    \
    --config_file nh_datasets/configs/segformer_xBD.py     \
    --train_dir data/xBD_tiled     --train_split train    \
    --val_dirs data/xBD_tiled data/CRASAR_tiled     \
    --val_split test
'''
...
class XBDDualSiameseModel(nn.Module):
    ...
    def forward(self, pixel_values_pre, pixel_values_post, labels=None, **kwargs):
        pixel_values_pre = pixel_values_pre.contiguous()
        pixel_values_post = pixel_values_post.contiguous()
